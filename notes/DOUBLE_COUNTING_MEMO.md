...
(`notes/CROSSING_BLOCK_SOURCE.md`, Appendix App01). That topology is
absent from SCBA.
...
5. **Library I internal disjointness is not Library II disjointness.**
   `notes/CROSSING_BLOCK_SOURCE.md` (“Disjointness from `B_BORN`”) only
...
- `notes/CROSSING_BLOCK_SOURCE.md` (App01 kernel; disjointness from
  `B_BORN` only; PDF `references/cond-mat-0607731-barisic-epjb-54-1.pdf`)
- `notes/SIGNED_CONVENTION_SOURCES.md` (Goodvin / Ciuchi / Barišić map)
...
Parent `experiments/FUTURE_B/FUTURE_B_C1_BLOCK_LIBRARY_FREEZE.md` remains
archive-only and is not live admission.

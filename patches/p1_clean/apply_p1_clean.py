#!/usr/bin/env python3
"""Historical P1_FIXTURE gating script. Not a public install step.

Requires a private C2_fixture adapter tree that is NOT in the public
snapshot. Missing source must not delete an existing destination.
"""
...
DEFAULT_SRC = ROOT / "research/mainline/CLOSURE/2026-09-14/runs/night1/C2_fixture/trees/adapter"
DEFAULT_DST = ROOT / "closure/2026-09-16-final/trees/p1_clean"
...
def apply_to_tree(src: Path) -> None:
    ...
    dump = dump.replace(
        "    if (stage == 1) n_stage1_rej = n_stage1_rej + 1\n",
    ...
    (src / "p1_clean.json").write_text(
        json.dumps(
            {
                "schema": "p1-clean-v1",
                "fixture_default": "off",
                "historical_only": True,
                "note": "dump and after-commit reverse only if P1_FIXTURE=1",
            },
            indent=2,
        )
        + "\n"
...
def main(argv=None) -> int:
    ...
    p.add_argument("--overwrite", action="store_true", help="replace an existing destination after a successful patch")
    args = p.parse_args(argv)
    src = args.src.resolve()
    dst = args.dst.resolve()
    if not src.is_dir():
        print(f"error: source adapter tree missing: {src}", file=sys.stderr)
        print("This script is historical-only; the public snapshot does not include the adapter.", file=sys.stderr)
        return 2
    if dst.exists() and not args.overwrite:
        print(f"error: destination exists, refusing to delete: {dst}", file=sys.stderr)
        return 3
    tmp = Path(tempfile.mkdtemp(prefix="p1_clean_"))
    try:
        shutil.copytree(src, tmp / "tree", ignore=shutil.ignore_patterns("*.o", "*.mod", "perturbo.x"))
        apply_to_tree(tmp / "tree" / "pert-src")
        if dst.exists():
            shutil.rmtree(dst)
        shutil.move(str(tmp / "tree"), str(dst))
    except Exception:
